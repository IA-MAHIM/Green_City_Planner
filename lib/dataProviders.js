...
export async function warmCityCache(lat, lng) {
  const tasks = [
    ...
    // ✅ NEW: warm water level too
    fetchWaterLevel(lat, lng, { cacheOnly: false }),
  ];
...
// ✅ NEW: data-driven Water Level proxy (soil moisture + last-24h rain)
export async function fetchWaterLevel(lat, lng, { cacheOnly = false } = {}) {
  const key = `water:${lat.toFixed(3)},${lng.toFixed(3)}`;
  const cached = cacheGet(key);
  if (cached) return cached;
  if (cacheOnly) return null;
  ...
  const url =
    `https://api.open-meteo.com/v1/forecast` +
    `?latitude=${lat}&longitude=${lng}` +
    `&hourly=soil_moisture_0_1cm,soil_moisture_1_3cm,soil_moisture_3_9cm,precipitation` +
    `&past_days=1&forecast_days=1&timezone=auto`;
  ...
  try {
    ...
    const times = H.time || [];
    if (!times.length) {
      const out = { indexPct: null, level: 'N/A', precip24h: null, soilTop: null, soilMid: null, soilDeep: null };
      cacheSet(key, out, 5 * 60 * 1000);
      return out;
    }
    const last = times.length - 1;

    // Normalize soil moisture (approx range 0–0.6 m3/m3) into 0–1
    const norm = v => v == null ? 0 : Math.max(0, Math.min(1, v / 0.6));
    const soilTop  = norm(H.soil_moisture_0_1cm?.[last]);
    const soilMid  = norm(H.soil_moisture_1_3cm?.[last]);
    const soilDeep = norm(H.soil_moisture_3_9cm?.[last]);

    // Composite moisture (top weighted higher for surface water)
    const moistureComposite = 0.5 * soilTop + 0.3 * soilMid + 0.2 * soilDeep;

    // 24h precipitation (mm)
    const precipArr = H.precipitation || [];
    const precip24h = Number(precipArr.slice(Math.max(0, precipArr.length - 24)).reduce((a, b) => a + (b || 0), 0).toFixed(1));

    // Composite water index 0–1: moisture (70%) + rain (30%; 50mm -> 1)
    const rainFactor = Math.max(0, Math.min(1, precip24h / 50));
    const waterIndex = 0.7 * moistureComposite + 0.3 * rainFactor;
    const indexPct = Number((waterIndex * 100).toFixed(0));

    let level = 'Low';
    if (waterIndex >= 0.45 || precip24h >= 50) level = 'High';
    else if (waterIndex >= 0.30 || precip24h >= 20) level = 'Moderate';

    const out = { indexPct, level, precip24h, soilTop, soilMid, soilDeep };
    cacheSet(key, out, 10 * 60 * 1000); // 10 min
    return out;
  } catch (e) {
    console.error("[WATER] FAIL", e);
    const out = { indexPct: null, level: 'N/A', precip24h: null, soilTop: null, soilMid: null, soilDeep: null };
